...
var _validateManifest = function(manifest, fn) {
  ...
  if (typeof manifest.api.production === 'undefined') {
    console.log('  check that production api exists', '[FAIL]'.fail);
    console.log();
    return fn(false);
  } else {
  ...
  if (typeof manifest.api.test === 'undefined') {
    console.log('  check for test api url', '[FAIL]'.fail);
    console.log();
    return fn(false);
  } else {
    console.log('  check for test api url', '[PASS]'.pass);
  }
...
// the result.
//
var _sendInvalidSso = function(id, fn) {
  var opts = {
      ...
      dash.validatePost(function() {
        dash.validateDelete(function() {
          dash.validatePut(function() {
            dash.validateSso(dash.id || dash.manifest.id);
          });
        });
      });
    } else {
