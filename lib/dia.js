...
var librarian = require('./librarian').init('xammr.com', 8686, false)
...
dia.validateDelete = function(fn) {
  ...
  console.log(util.format('testing DELETE to %s/%d', dia.manifest.api.test.base_url, dia.id).info);
...
dia.validatePut = function(fn) {
  ...
  console.log(util.format('testing PUT to %s/%d', dia.manifest.api.test.base_url, dia.id).info);
...
dia.authenticate = function(username, password, fn) {
  librarian.user.authenticate(username, librarian.util.createHash(password), function(err, user) {
    if (err) {
      var token = null
        , user = new Buffer(util.format('%s:%s', username, password), 'ascii').toString('base64');
